WITH
  ...
  PAG AS (
    SELECT
      X.IDEMPRESA,
      X.IDPLANILHA,
      LISTAGG (VARCHAR(X.IDRECEBIMENTO), ',') WITHIN GROUP (
        ORDER BY
          X.IDRECEBIMENTO
      ) AS IDRECEBIMENTO,
      LISTAGG (NULLIF(TRIM(X.DESCRRECEBIMENTO), ''), ' | ') WITHIN GROUP (
        ORDER BY
          X.IDRECEBIMENTO
      ) AS DESCRRECEBIMENTO
    FROM
      (
        ...
        FROM
          ...
          LEFT JOIN DBA.FORMA_PAGREC FP ON FP.IDRECEBIMENTO = CR.IDRECEBIMENTO
      ) X
    GROUP BY
      X.IDEMPRESA,
      X.IDPLANILHA
  ...
  ITENS AS (
    SELECT
      ...
      O.DTMOVIMENTO,
      /* >>> CAMPOS CABEÇALHO PEDIDO <<< */
      O.OBSERVACAO,
      REPLACE (
        CASE
          WHEN LENGTH (O.OBSERVACAO) > 0 THEN O.OBSERVACAO
          ELSE ''
        END,
        '<E>',
        ' '
      ...
      (
        SELECT
          CF2.NOME
        FROM
          DBA.CLIENTE_FORNECEDOR CF2
        WHERE
          CF2.IDCLIFOR = OP.IDVENDEDOR
      ) AS NOMEVENDEDOR,
      OP.FLAGCANCELADO,
      /* chaves da prenota */
      ...
      (
        ...
        FROM
          DBA.ORCAMENTO_ROTEIRO_PROD ORP
          JOIN DBA.ORCAMENTO_PROD OP2 ON OP2.IDORCAMENTO = ORP.IDORCAMENTO
          ...
          JOIN DBA.ROTEIRO_ENTREGA_LOCAL REL ON REL.IDROTEIRO = ORP.IDROTEIRO
          AND REL.IDLOCALRETIRADA = OP2.IDLOCALRETIRADA
          AND REL.ORDEM = 1
          JOIN DBA.LOCAL_RETIRADA LR ON LR.IDLOCALRETIRADA = REL.IDLOCALRETIRADA
          JOIN DBA.ESTOQUE_CADASTRO_LOCAL ECL ON ECL.IDLOCALESTOQUE = LR.IDLOCALESTOQUE
          AND ECL.IDEMPRESABAIXAEST = OP2.IDEMPRESA
        WHERE
          ORP.IDPRODUTO = OP.IDPRODUTO
    ...
    FROM
      DBA.ORCAMENTO O
      JOIN DBA.ORCAMENTO_PROD OP ON O.IDEMPRESA = OP.IDEMPRESA
      AND O.IDORCAMENTO = OP.IDORCAMENTO
      JOIN DBA.PRODUTO_GRADE PG ON PG.IDPRODUTO = OP.IDPRODUTO
      AND PG.IDSUBPRODUTO = OP.IDSUBPRODUTO
      /* join do PRODUTOS_VIEW pra pegar FABRICANTE e CODBARRAS unitário */
      LEFT JOIN DBA.PRODUTOS_VIEW PVF ON PVF.IDSUBPRODUTO = OP.IDSUBPRODUTO
      ...
      LEFT JOIN LATERAL (
        SELECT
          ...
          END AS CODBARCX
        FROM
          DBA.PRODUTO_GRADE_CODBARCX PGCX
        WHERE
          PGCX.IDPRODUTO = OP.IDPRODUTO
      ...
      LEFT JOIN DBA.PRODUTO PR ON PR.IDPRODUTO = OP.IDPRODUTO
      LEFT JOIN DBA.SECAO S ON S.IDSECAO = PR.IDSECAO
      LEFT JOIN DBA.CLIENTE_FORNECEDOR CF ON CF.IDCLIFOR = O.IDCLIFOR
      LEFT JOIN DBA.ORCAMENTO_PRE_NOTA OPREN ON O.IDEMPRESA = OPREN.IDEMPRESAORCAMENTO
    ...
    WHERE
      ...
      AND NOT EXISTS (
        SELECT
          1
        FROM
          DBA.ORCAMENTO_INFORMACAO_ADICIONAL ORCAD
        WHERE
          ORCAD.IDEMPRESA = O.IDEMPRESA
...
FROM
  ITENS I
  JOIN DBA.LOCAL_RETIRADA LRRET ON LRRET.IDLOCALRETIRADA = COALESCE(I.IDLOCALRETIRADA_ROT, I.IDLOCALRETIRADA_ORC)
  AND LRRET.FLAGBLOLOCALRETAGUARDA = 'F'
  LEFT JOIN PAG P ON P.IDEMPRESA = I.IDEMPRESAPRENOTA
  AND P.IDPLANILHA = I.IDPLANILHAPRENOTA FOR READ ONLY;