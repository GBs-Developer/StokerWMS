...
export async function seedDatabase() {
  ...
  const [supervisor] = await db.insert(users).values({
    ...
    active: true,
    badgeCode: generateBadgeCode("admin", "admin123"),
  }).returning();
  ...
  const [separador] = await db.insert(users).values({
    ...
    active: true,
    badgeCode: generateBadgeCode("joao", "1234"),
  }).returning();
  ...
  const [conferente] = await db.insert(users).values({
    ...
    active: true,
    badgeCode: generateBadgeCode("maria", "1234"),
  }).returning();
  ...
  const [balcao] = await db.insert(users).values({
    ...
    role: "balcao" as any,
    active: true,
    badgeCode: generateBadgeCode("pedro", "1234"),
  }).returning();
